...
Example: Just want to get text from a single page (e.g. Page 37)? 
```{r single-page}
got_p37 <- got_text[37]
got_p37
```

See how that compares to the text in the PDF on Page 37. What has pdftools added and where?
#Answer: For every line break it has added \n to indicate that there is a line break
...
Or we can summarize sentiment for the data: 
```{r summarize-afinn}
...
Wait, so "lord" is showing up in NRC lexicon as "disgust, negative, positive and trust"? Let's check:
```{r nrc-confidence}
...
#Answer: Some of the most common words are "lord", "ser", "jon" and "ned". These words are all ambiguous words with a sentiment, which is hard to define definitly. The word "lord" is ambiguous because it can have both a negative and positive sentiment depending on the context and situation in which it is used
